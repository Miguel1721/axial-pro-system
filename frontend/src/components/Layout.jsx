 ...
 * - Soporte para reducción de animaciones (accesibilidad)
 */
...
import { LogOut, LayoutDashboard, Calendar, Users, Stethoscope, CreditCard, Package, FileText, Video } from 'lucide-react';

// Componentes importados (se pueden memoizar individualmente)
...
// Configuración de menús memoizada
const MENU_ITEMS_CONFIG = {
  admin: [
    ...
    { path: '/telemedicina', icon: 'Video', label: 'Telemedicina' }
  ],
  medico: [
    { path: '/dashboard', icon: 'LayoutDashboard', label: 'Panel' },
    { path: '/agenda', icon: 'Calendar', label: 'Agenda' },
...
const Layout = () => {
  ...
  // Memoizar menú del usuario para evitar recalcular en cada render
  const userMenuItems = useMemo(() => {
    return MENU_ITEMS_CONFIG[user?.rol] || [];
  }, [user?.rol]);

  // Memoizar label de página actual
  const currentPageLabel = useMemo(() => {
    return userMenuItems.find(
      item => location.pathname === item.path || location.pathname.startsWith(item.path)
    )?.label || (user?.rol === 'paciente' ? 'Mi Portal' : 'Panel');
  }, [location.pathname, userMenuItems, user?.rol]);
  ...
  // Memoizar estilos de animación según preferencias de accesibilidad
  const animationClasses = useMemo(() => {
    return reduceMotion ? '' : 'transition-colors duration-300';
  }, [reduceMotion]);

  return (
    <div className={`min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 ${animationClasses}`}>
      {/* PWA Install Prompt */}
      <PWAInstallPrompt />
      ...
      <div className="md:hidden pt-16 pb-20 px-4">
        <MobileContent user={user} currentPageLabel={currentPageLabel} />
      </div>
...
const MobileHeader = memo(({ user, isDark }) => (
...
const DesktopHeader = memo(({ user, userMenuItems, isDark, location, handleLogout }) => (
...
const DesktopNav = memo(({ userMenuItems, location }) => {
  // Importar iconos dinámicamente para optimizar bundle
  const icons = {
    LayoutDashboard, Calendar, Users, Stethoscope, CreditCard, Package, FileText, Video
  };
...
const DesktopControls = memo(({ user, handleLogout }) => (
  <div className="flex items-center space-x-3">
...
const MobileContent = memo(({ user, currentPageLabel }) => (
...
const MobileBottomNav = memo(({ userMenuItems, location, handleLogout, isDark }) => {
  const icons = {
    LayoutDashboard, Calendar, Users, Stethoscope, CreditCard, Package, FileText, Video
  };
      ...
      <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl border-t border-gray-200/50 dark:border-gray-700/50 pb-safe">
        <div className="flex items-center justify-around px-2 py-2">
          ...
          {userMenuItems.length > 4 && (
            <button className="flex flex-col items-center justify-center px-3 py-2 rounded-xl text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700">
